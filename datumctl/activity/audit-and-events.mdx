...
For anything the shorthand flags do not cover, `--filter` takes a server-side expression that is applied on top of them — for example `--filter "responseStatus.code >= 400"` to find failed requests.
