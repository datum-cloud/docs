...
description: "Deploy a container image as a workload with datumctl compute deploy, publish it on an HTTPS URL, and check its health with datumctl compute workloads."
---
...
If a workload named `api` doesn't exist, this command creates it with one placement named `default` that spans both locations, runs at least two Instances in each location, and publishes the workload on a Datum-managed HTTPS URL. If you run the same command again with a new `--image`, it updates the existing workload. Like `datumctl apply`, `deploy` creates or updates.
...
| `--http-port` | Port the container serves plain HTTP on. Publishes the workload on a Datum-managed HTTPS URL. |
| `--no-http` | Remove the workload's URL. |
...
`--http-port` declares the port your container serves plain HTTP on, and publishes the workload on a Datum-managed HTTPS URL. Datum terminates TLS, so don't serve HTTPS on that port. After the rollout, `deploy` prints the progress of publishing and then the URL, on its own line, as the last line of output:
...
Redeploying without `--http-port` keeps the workload's current port and URL. To remove the URL, pass `--no-http`. Without an HTTP port, the workload isn't reachable from the internet. For custom domains and publishing workloads you manage with manifests, see [Publish a workload](/compute/publish-workloads).
...
A manifest deploy doesn't create or remove a URL. You can't combine `-f` with `--http-port`, `--no-http`, `--runtime-class`, `--network`, or `--build`. Set the runtime class and network in the manifest instead.
...
The `HEALTH` column shows `Available`, `Degraded`, `Unavailable`, or `Unknown`. The `--health` flag takes the lowercase form, such as `--health=degraded`. The `URL` column shows each workload's URL. For the full picture on a single workload — its container spec, scale settings, URL, and per-location ready/desired counts together — use `describe`:
