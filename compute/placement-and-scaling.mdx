...
The flags put all locations in a single placement named `default`. On an existing workload, a flag-based deploy replaces every placement and some other settings. For more information, see [Update a workload](/compute/manage-workloads#update-a-workload).
