...
description: "Deploy a public container image to Dallas and reach it at a Datum-managed HTTPS URL, then clean up."
---

In this quickstart, you deploy a small web server from a public container image to the Dallas location, reach it at a public HTTPS URL, and then delete it.
...
<Steps>
  <Step title="Deploy the workload">
    ...
    - `--http-port=80`: publishes port `80` of the container on a Datum-managed HTTPS URL.
    ...
    The last line is your workload's URL. Your hostname is different. The command also saves the workload definition to `workload.yaml` in the current directory.
  </Step>
...
1. To delete the workload, its Instance, and its URL, run the following command:
