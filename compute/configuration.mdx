...
You can set environment variables, and load or mount whole ConfigMaps and Secrets, with `datumctl compute deploy` flags. These flags require compute plugin v0.10.7 or later. For anything that the flags can't set, such as a variable from one key of a Secret or credentials for a private registry, write a workload manifest. Apply the manifest with `datumctl compute deploy -f FILE` or `datumctl apply -f FILE`, where `FILE` is the path to your manifest. You can't combine the configuration flags with `-f`. A flag-based deploy saves its result to `workload.yaml`, which you can edit as a starting point. Each flag-based deploy overwrites `workload.yaml` in the current directory, so keep edited manifests under another name.

<Warning>
  A flag-based `datumctl compute deploy` replaces the workload's whole template and placements with what the flags describe. It keeps environment variables, ConfigMap and Secret imports, volumes, and labels. It removes registry credentials, custom commands, and extra placements that you set in a manifest, and it sets the Instance count to the `--min` value, which defaults to `1`. After you add settings that the flags can't set, update the workload with `datumctl compute deploy -f FILE`.
</Warning>

## Set environment variables

To set an environment variable to a fixed value, pass `--env KEY=VALUE` when you deploy. Repeat the flag for each variable. Values can contain commas. The following command sets two variables:
...
datumctl compute deploy WORKLOAD_NAME \
  ...
  --location=LOCATION \
  --env=LOG_LEVEL=info \
  --env=ALLOWED_ORIGINS=a.example,b.example
```
...
Variables carry forward. A later deploy that doesn't name a variable keeps its value. To remove a variable, add a trailing `-`, such as `--env=LOG_LEVEL-`.

In a manifest, add the variable to the container's `env` list, as in the following example:
...
The `deploy` flags can't set one variable from one key, so use a manifest. To set one environment variable from one key, use `valueFrom` with `configMapKeyRef` or `secretKeyRef`. This method works with both runtime classes. The following example sets one variable from a Secret key and one from a ConfigMap key:
...
On `general-purpose` workloads, you can turn every key of a ConfigMap or Secret into an environment variable. To do so with flags, pass `--env-from-configmap` or `--env-from-secret` as `NAME` or `NAME:PREFIX_`. The following command loads every key from `api-config` with the prefix `APP_`, and every key from `api-secrets` without a prefix:

```bash
datumctl compute deploy WORKLOAD_NAME \
  --image=IMAGE \
  --location=LOCATION \
  --env-from-configmap=api-config:APP_ \
...
To remove an import, add a trailing `-` to the name, such as `--env-from-secret=api-secrets-`.

In a manifest, use `envFrom`. Each entry names exactly one source. The optional `prefix` is added to each variable name, and must be a valid variable name itself. The following manifest example loads the same keys:
...
To mount a ConfigMap or Secret as files with flags, pass `--configmap` or `--secret` as `NAME:/mount/path`. Each key becomes a file in the mount directory. The following command mounts `api-config` at `/etc/api` and `api-secrets` at `/etc/api-secrets`:

```bash
datumctl compute deploy WORKLOAD_NAME \
  --image=IMAGE \
  --location=LOCATION \
  --configmap=api-config:/etc/api \
  --secret=api-secrets:/etc/api-secrets
```
...
To remove a mount, add a trailing `-` to the name, such as `--secret=api-secrets-`.

In a manifest, declare a volume in `spec.template.spec.volumes`, and attach it to a container with `volumeAttachments`.

The following manifest example mounts the same ConfigMap and Secret:
